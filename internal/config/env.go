...
//
// Используется для установки сетевого адреса из переменных окружения.
// Реализуется типом NetAddress.
type AddrSetter interface {
	// Set устанавливает адрес из строки вида "host:port".
	// Возвращает ошибку, если формат строки некорректный.
	Set(string) error
}
